...
export async function callClaude({ blocks, numQ, diff, type, uiLangName, learnerBrief, withSummary }) {
  const typeMap = {
    ...
    diagram: `Diagram questions on the image the user uploaded (their real diagram/figure). For EACH question pick ONE distinct part or region, and give its location as "x" and "y": numbers 0 to 100 for the CENTER of that part as a percentage of the image width (x, 0=left 100=right) and height (y, 0=top 100=bottom), so a marker can be drawn on it, be accurate. Then ask a question ABOUT the marked part with 4 plausible "options", "correct" (0-based index), and "answer" (the correct option's exact text). CRITICAL: if the diagram ALREADY LABELS its parts (names printed on the image), do NOT ask "what is this part", the answer would be sitting right next to the marker; instead test real understanding of the marked part, its FUNCTION or role, what it connects to or interacts with, its step or order in the process, what happens if it is removed or fails, or how it differs from another part. Only ask a plain "what is this" identification question when the diagram is UNLABELED. Never write a question that can be answered just by reading text printed in the image. Vary the question types across the set, and do not mark the same part twice.`,
  };
