...
export async function callClaude({ blocks, numQ, diff, type, uiLangName, learnerBrief, withSummary }) {
  const typeMap = {
    ...
    diagram: `Diagram questions on the image the user uploaded (their real diagram/figure). For EACH question pick ONE distinct part or region and ask a question ABOUT it with 4 plausible "options", "correct" (0-based index), and "answer" (the correct option's exact text). There is NO marker on the image, so identify the part clearly WITHIN the question by describing its position or appearance (e.g. "the structure at the top of the diagram", "the arrows rising from the ocean", "the large central organelle"). CRITICAL: if the diagram ALREADY LABELS its parts (names printed on the image), do NOT ask "what is this part", the answer would be printed right on it; instead test real understanding, its FUNCTION or role, what it connects to or interacts with, its step or order in the process, what happens if it is removed or fails, or how it differs from another part. Only ask a plain "what is this" identification question when the diagram is UNLABELED. Never write a question that can be answered just by reading text printed in the image. Vary the question types across the set, and do not ask about the same part twice.`,
  };
  const wantsCoords = false; // diagram no longer uses an on-image marker; the question names the part
